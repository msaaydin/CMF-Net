...
from helpers import read_excel_data_zero_echo as read_excel_data
...
class MyDataset(Dataset):
    def __init__(self, root_dir, split, transform=None):
        ...
        self.labels = []
        self.text_features = []
        self.text_data = read_excel_data("data_old.xlsx")
...
# data_dir = 'data'
data_dir = 'C:/Users/FSM/Desktop/MUSA/SamsunTip/Musa Hoca/data'
...
class CombinedModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, seed, combined_hidden_size = 128, model_name=None):
        ...
        self.cnn_branch.load_state_dict(torch.load(f"C:/Users/FSM/Desktop/MUSA/SamsunTip/Musa Hoca/standart_cnn/model_{model_name}_oneoutput_{seed}.pth", map_location=device))
        # Freeze CNN layers
        for param in self.cnn_branch.parameters():
            param.requires_grad = False
        self.cnn_branch = self.cnn_branch.to(device)

        self.mlp_branch = MLPBranch(input_size, hidden_size, output_size)
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.cnn_featureNum + 128, combined_hidden_size), 
            nn.GELU(),
            ...
            nn.Linear(combined_hidden_size, combined_hidden_size),
            nn.GELU(),
            nn.Linear(combined_hidden_size, 1),
        )
        #self.fc_combined = self.fc_combined.to(device)  # Combine CNN (128) and MLP (32) features

    def forward(self, image, clinical_data):
        cnn_features = my_forward(image, self.model_name, self.cnn_branch)
        mlp_features = self.mlp_branch(clinical_data)
        combined_features = torch.cat((cnn_features, mlp_features), dim=1)  # Concatenate features
        
        output = self.classifier(combined_features)  # Binary classification
        return output
...
for ii in seed_list:
    ...
    for model_name, model_func in models_dict.items():
        ...
        model = CombinedModel(input_size, hidden_size, output_size, ii, model_name=model_name)
        model.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            if val_f1 > best_val_f1:
                best_val_f1 = val_f1
                torch.save(model.state_dict(), f'late_fusion/{model_name}_late_fusion_{ii}.pth')
                print('Saved new best model')

        # Testing phase
        model.load_state_dict(torch.load(f'late_fusion/{model_name}_late_fusion_{ii}.pth'))
        model.eval()
        ...
        plt.savefig(f'late_fusion/model_{model_name}_{ii}.png')
        plt.close()

        result_txt = f"Accuracy: {accuracy}\nPrecision: {precision}\nRecall: {recall}\nF1-Score: {f1}"

        with open(f"late_fusion/{model_name}_result_{ii}.txt", "w") as f:
            f.write(result_txt)